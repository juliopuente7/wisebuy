...
const C = {
  bg: "#F5F7FA",       // hueso claro
  bg2: "#EEF2F7",
  card: "#FFFFFF",     // tarjetas blancas
  cardHi: "#F5F7FA",
  ink: "#16294A",      // texto navy (oscuro)
  sub: "#5B6472",      // gris texto
  faint: "#98A2B3",
  line: "#E4E7EC",     // líneas claras
  lineHi: "#D3D9E0",
  navy: "#16294A",     // azul oscuro (confianza)
  navy2: "#1E3A5F",
  yellow: "#F79420",   // naranja claro (para gradientes)
  yellowDeep: "#F2610C",
  orange: "#F26419",
  orangeDeep: "#E0550C",
  ok: "#12B76A",       // verde
  warn: "#E23B3B",     // rojo alerta
  red: "#E23B3B",
  redDeep: "#B71C1C",
  warnSoft: "#FDE9E9", // fondo alerta claro
  amberSoft: "#FDEEE2",
  // fondos de color tenue claros
  panelYellow: "#FEF6E7",
  panelOrange: "#FDEEE2",
  panelGreen: "#E3F7EE",
  field: "#FFFFFF",      // campos de texto blancos
  fieldInk: "#16294A",   // texto dentro de los campos
  fieldLine: "#E4E7EC",
};
  ...
  <svg width={size} height={size} viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: "block" }}>
    <defs>
      <linearGradient id="wbGrad" x1="0" y1="0" x2="120" y2="120" gradientUnits="userSpaceOnUse">
        <stop stopColor="#F79420" /><stop offset="1" stopColor="#F26419" />
      </linearGradient>
    </defs>
...
.wb-legal h1{font-family:Georgia,serif;font-size:20px;color:#E0550C;border-bottom:2px solid #F26419;padding-bottom:8px;margin-top:26px}
.wb-legal h1:first-child{margin-top:0}
.wb-legal h2{font-family:Georgia,serif;font-size:17px;color:#E0550C;margin-top:26px;border-bottom:1px solid #eee;padding-bottom:5px}
...
<hr></div>`;
  ...
  const PRECIO = "19,99 €";
  // ▼▼▼ TU FOTO: pega aquí la dirección (URL) de tu foto de peritos y aparecerá en la portada.
  //     Si lo dejas vacío (""), se muestra un diseño elegante sin foto.
  const FOTO_PERITOS = "";
  const DOMUSCAR_URL = "https://domuscarperitos.com";
  ...
  // Miniatura pequeña (dataURL) para adjuntar al informe y que persista siempre
  const miniFoto = (f) => new Promise((resolve, reject) => {
    const img = new Image();
    const url = URL.createObjectURL(f);
    img.onload = () => {
      const MAX = 560;
  ...
  const buildGuiaHTML = () => {
    ...
    const boxR = '<span style="display:inline-block;width:12px;height:12px;border:1.5px solid #E23B3B;border-radius:3px;margin-right:8px;vertical-align:-1px"></span>';
    let h = '';
...
<style>
  *{box-sizing:border-box} html,body{margin:0;padding:0}
  ...
  .brand{display:flex;align-items:center;gap:12px;padding:18px 0 14px;border-bottom:3px solid;border-image:linear-gradient(90deg,#F79420,#F26419) 1}
  ...
  h2{font-family:Georgia,serif;font-size:17px;margin:22px 0 8px;color:#E0550C;border-bottom:1px solid #eee;padding-bottom:5px}
  ...
  .btn{display:inline-block;margin:14px 0 4px;background:linear-gradient(135deg,#F2610C,#E0550C);color:#fff;text-decoration:none;padding:11px 18px;border-radius:9px;font-weight:700;font-size:14px}
...
<div class="brand">
  <div class="logo"><svg width=\"40\" height=\"40\" viewBox=\"0 0 120 120\" xmlns=\"http://www.w3.org/2000/svg\"><defs><linearGradient id=\"g\" x1=\"0\" y1=\"0\" x2=\"120\" y2=\"120\" gradientUnits=\"userSpaceOnUse\"><stop stop-color=\"#F79420\"/><stop offset=\"1\" stop-color=\"#F26419\"/></linearGradient></defs><rect width=\"120\" height=\"120\" rx=\"27\" fill=\"#FFFFFF\"/><path d=\"M35 58 L60 38 L85 58 L85 88 Q85 90 83 90 L37 90 Q35 90 35 88 Z\" fill=\"none\" stroke=\"url(#g)\" stroke-width=\"6.5\" stroke-linejoin=\"round\"/><path d=\"M49 66 L57 74 L75 54\" fill=\"none\" stroke=\"url(#g)\" stroke-width=\"6.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/></svg></div>
...
<title>Informe de compra · WiseBuy</title><style>
  *{box-sizing:border-box}
  ...
  h2{font-family:Georgia,serif;font-size:17px;margin:24px 0 8px;color:#E0550C;border-bottom:1px solid #Eee;padding-bottom:5px}
  ...
  tr.total td{background:#FBF2E0;font-size:14px;border-top:2px solid #F26419}
  .msg{white-space:pre-wrap;background:#fff;border:1px solid #ddd;border-left:3px solid #F26419;border-radius:8px;padding:14px;font-size:13.5px}
  .btn{display:inline-block;margin:14px 0 4px;background:linear-gradient(135deg,#F2610C,#E0550C);color:#fff;text-decoration:none;padding:11px 18px;border-radius:9px;font-weight:700;font-size:14px}
  ...
  .foot .shield{color:#E0550C;font-weight:700}
  @media print{
...
<div class="foot"><span class="shield">🛡️ Revisado por IA entrenada por peritos profesionales.</span> Estimación orientativa; no sustituye una inspección presencial ni un informe técnico firmado. Confirma el estado real y la documentación antes de comprar.</div>
  ...
  const analizar = async () => {
    ...
    try {
      ...
      for (const f of anuncio) { const im = await prepararImagen(f); blocks.push({ type: "image", source: { type: "base64", media_type: im.media_type, data: im.data } }); try { fotosA.push(await miniFoto(f)); } catch (_) {} }
      if (visita.length) blocks.push({ type: "text", text: "== FOTOS DE LA VISITA PRESENCIAL (más fiables, priorízalas) ==" });
      for (const f of visita) { const im = await prepararImagen(f); blocks.push({ type: "image", source: { type: "base64", media_type: im.media_type, data: im.data } }); try { fotosV.push(await miniFoto(f)); } catch (_) {} }
...
OBLIGATORIO: rellena SIEMPRE estos campos, nunca los dejes vacíos: cada partida DEBE tener "urgencia" ("alta", "media" o "baja"), "total" (número, = cantidad × precio_unitario), "detalle", "consecuencia" (qué pasa si no se arregla) y "gremio" (a quién llamar). "sobrecoste_total_estimado" DEBE ser la suma de los totales de las partidas. "coste_real_estimado" = precio_venta + sobrecoste_total_estimado. "veredicto" DEBE ser "buena", "ajustada" o "cara", con "veredicto_texto" explicándolo. "recomendacion" DEBE ser "recomendable", "con_precaucion" o "no_recomendable", con "recomendacion_razones" (3 razones). Rellena "costes_compra" (impuestos ITP/IVA según sea 2ª mano u obra nueva y comunidad autónoma, notaría/registro/gestoría, total) y "gastos_anuales" (IBI, comunidad, suministros estimados) con importes orientativos realistas. Rellena "comparativa_precio" con precio_pedido, precio_mercado (m² × precio/m² de la zona), diferencia_pct y un texto. Si no hay reformas, pon partidas:[] y sobrecoste_total_estimado:0.`;
      ...
      // Solo se marca "incompleto" si la IA devuelve algo LITERALMENTE vacío
      // (un objeto sin ningún dato). Cualquier informe con contenido cobra y no avisa.
      const truncado = !parsed || typeof parsed !== "object" || Object.keys(parsed).length === 0;
      ...
      parsed.__fotosA = fotosA;
      parsed.__fotosV = fotosV;
      setResult(parsed);
      setTab("informe");
  ...
  const urg = {
    alta: { c: C.warn, l: "Prioritario", icon: "🔴", bg: "#FDE9E9", bd: "#E23B3B" },
    media: { c: C.yellow, l: "Medio plazo", icon: "🟡", bg: "#FDEEE2", bd: "#F79420" },
    baja: { c: C.ok, l: "Preventivo", icon: "🟢", bg: "#E3F7EE", bd: "#4FD08A" },
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.ink, fontFamily: sans }}>
      ...
      <div className="no-print" style={{ borderBottom: `2px solid transparent`, borderImage: `linear-gradient(90deg, ${C.yellow}, ${C.orange}) 1`, background: C.navy, position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "14px 28px", display: "flex", alignItems: "center", gap: 11 }}>
          <div onClick={reset} title="Volver al inicio" style={{ display: "flex", alignItems: "center", gap: 11, cursor: "pointer" }}>
            <LogoMark size={28} />
            <span style={{ fontWeight: 700, fontSize: 16, letterSpacing: -0.2, color: "#fff" }}>WiseBuy</span>
          </div>
          <span style={{ fontSize: 11.5, color: "#AEB9CC", borderLeft: "1px solid #33456A", paddingLeft: 10, marginLeft: 2 }}>Compra vivienda sin sorpresas</span>
          <span style={{ marginLeft: "auto", fontSize: 11, color: "#AEB9CC", fontFamily: mono, letterSpacing: 0.5 }}>ES</span>
        </div>
      </div>

      {!result && !verificandoPago && (
        <div style={{ maxWidth: 960, margin: "0 auto", padding: "28px 20px 6px", position: "relative", zIndex: 1 }}>
          <div className="hero-box" style={{ display: "grid", gridTemplateColumns: "1.12fr 0.88fr", background: "linear-gradient(180deg,#ffffff,#EEF2F7)", border: `1px solid ${C.line}`, borderRadius: 22, overflow: "hidden", boxShadow: "0 10px 34px rgba(22,41,74,.09)" }}>
            <div style={{ padding: "42px 38px" }}>
              <span style={{ display: "inline-block", fontFamily: mono, fontSize: 11.5, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", color: C.orange, background: C.amberSoft, padding: "5px 12px", borderRadius: 999, marginBottom: 16 }}>Tu perito de bolsillo</span>
              <h1 style={{ fontFamily: serif, fontSize: "clamp(28px,5vw,42px)", fontWeight: 700, lineHeight: 1.08, letterSpacing: -0.5, margin: 0, color: C.navy }}>
                Visita tu futura casa con <span style={{ color: C.orange, fontStyle: "italic" }}>criterio</span>
              </h1>
              <p style={{ color: C.sub, fontSize: 16, marginTop: 14, maxWidth: 440, lineHeight: 1.55 }}>
                Sube las fotos del piso y te decimos qué defectos tiene, cuánto costará arreglarlos y cuánto puedes negociar. Antes de firmar nada.
              </p>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, marginTop: 20, fontSize: 13, color: C.sub }}>
                🛡️ Revisado por <span style={{ color: C.orange, fontWeight: 700 }}>IA</span> entrenada por <b style={{ color: C.navy }}>peritos profesionales</b>
              </div>
            </div>
            <div className="hero-panel" style={{ position: "relative", background: "radial-gradient(120% 120% at 70% 20%,#25406B 0%,#16294A 70%)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", minHeight: 230 }}>
              <div style={{ position: "absolute", width: 230, height: 230, borderRadius: "50%", border: "2px solid #2E4A78", top: -40, right: -60 }} />
              <div style={{ position: "absolute", width: 150, height: 150, borderRadius: "50%", background: "rgba(242,100,25,.16)", bottom: -30, left: -30 }} />
              <div style={{ position: "relative", zIndex: 2, textAlign: "center", color: "#fff", padding: 20 }}>
                <svg width="100" height="100" viewBox="0 0 120 120" fill="none" style={{ filter: "drop-shadow(0 8px 22px rgba(0,0,0,.35))" }}>
                  <path d="M24 56 L60 26 L96 56 L96 96 L24 96 Z" stroke="#fff" strokeWidth="6.5" strokeLinejoin="round" />
                  <path d="M44 66 L56 78 L80 50" stroke="#F79420" strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                <div style={{ fontFamily: serif, fontSize: 23, fontWeight: 700, marginTop: 10 }}>Wise<span style={{ color: "#F79420" }}>Buy</span></div>
                <div style={{ fontSize: 11.5, color: "#AEB9CC", marginTop: 4, letterSpacing: 0.4 }}>VIVIENDA VERIFICADA CON CRITERIO</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="app-main" style={{ maxWidth: 960, margin: "0 auto", padding: "20px 28px 96px", position: "relative", zIndex: 1 }}>
          ...
          <>
            ...
            <header style={{ marginBottom: 30 }}>
              ...
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, marginTop: 16, background: C.card, border: `1px solid ${C.line}`, borderRadius: 999, padding: "7px 14px" }}>
                ...
                <span style={{ fontSize: 12.5, color: C.ink, fontWeight: 600 }}>Revisado por <span style={{ fontFamily: mono, fontWeight: 700, fontSize: 13, letterSpacing: 0.5, color: "#F26419", textShadow: "0 0 9px rgba(45,226,230,0.65)" }}>IA</span> entrenada por peritos profesionales</span>
              </div>
            </header>
            ...
            <section className="cred-box" style={{ marginTop: 44, background: C.navy, borderRadius: 20, overflow: "hidden", display: "grid", gridTemplateColumns: FOTO_PERITOS ? "0.85fr 1.15fr" : "1fr" }}>
              {FOTO_PERITOS && (<div style={{ minHeight: 250, backgroundImage: `url('${FOTO_PERITOS}')`, backgroundSize: "cover", backgroundPosition: "center", backgroundColor: "#0f1d34" }} />)}
              <div style={{ padding: "34px 36px", color: "#fff" }}>
                <div style={{ fontFamily: mono, fontSize: 11.5, letterSpacing: 1.5, color: "#F79420", marginBottom: 12, textTransform: "uppercase" }}>Quiénes estamos detrás</div>
                <h2 style={{ fontFamily: serif, fontSize: 26, fontWeight: 700, margin: 0, lineHeight: 1.2 }}>Una herramienta creada por peritos de verdad</h2>
                <p style={{ color: "#C9D2E0", fontSize: 15, lineHeight: 1.6, marginTop: 14 }}>
                  WiseBuy nace de <b style={{ color: "#fff" }}>Domuscar Peritos</b>, una red de peritos profesionales que cada día valora daños y viviendas para <b style={{ color: "#fff" }}>empresas de reformas, aseguradoras e inspecciones particulares</b>. Hemos volcado ese criterio en una herramienta con inteligencia artificial, para que cualquiera pueda revisar una vivienda antes de comprarla con la misma mirada con la que lo haría un perito.
                </p>
                <a href={DOMUSCAR_URL} target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 8, marginTop: 18, background: "linear-gradient(135deg, #F79420, #F26419)", color: "#fff", fontWeight: 700, fontSize: 14.5, padding: "12px 22px", borderRadius: 11, textDecoration: "none" }}>
                  ¿Necesitas un perito presencial? → Domuscar Peritos
                </a>
              </div>
            </section>

            <section style={{ marginTop: 44 }}>
              <h2 style={{ fontFamily: serif, fontSize: 24, fontWeight: 700, color: C.navy, marginBottom: 10 }}>Comprar tu primera vivienda sin que te la cuelen</h2>
              <div style={{ display: "grid", gap: 14, color: C.sub, fontSize: 15, lineHeight: 1.65, maxWidth: 760 }}>
                <p>Comprar una casa es una de las decisiones más importantes de tu vida, y casi siempre se hace con el corazón. Nos enamoramos de la cocina, de la luz del salón, del barrio… y bajamos la guardia justo cuando más atentos deberíamos estar.</p>
                <p><b style={{ color: C.ink }}>WiseBuy está para bajar la emoción y subir el criterio.</b> Te ayudamos a ver lo que no se ve en las fotos bonitas del anuncio: humedades, instalaciones viejas, gastos que llegarán en unos años, si el precio es justo para la zona y cuánto puedes negociar.</p>
                <p>No se trata de que no compres, sino de que <b style={{ color: C.ink }}>compres con los ojos abiertos</b>, sabiendo exactamente en qué te metes y qué te va a costar de verdad. Sin sorpresas, sin sustos y sin arrepentimientos.</p>
                <p>Con nuestro informe llegas a la mesa de negociación sabiendo qué pedir, qué exigir que reparen y cuánto rebajar. Lo que antes solo estaba al alcance de un profesional, ahora lo tienes tú.</p>
              </div>
            </section>
          </>
        )}
        ...
        <style>{`
          ...
          .hero-box{grid-template-columns:1.12fr 0.88fr}
          @media(max-width:720px){ .hero-box{grid-template-columns:1fr !important} .hero-panel{display:none !important} .cred-box{grid-template-columns:1fr !important} }
          ...
          <div style={{ background: C.card, border: `1px solid ${C.orange}55`, borderRadius: 16, overflow: "hidden", marginBottom: 4 }}>
            ...
            <div style={{ padding: "26px 24px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 11.5, color: C.sub, marginTop: 6, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                🛡️ Revisado por <span style={{ fontFamily: mono, fontWeight: 700, letterSpacing: 0.5, color: "#F26419", textShadow: "0 0 8px rgba(45,226,230,0.6)" }}>IA</span> entrenada por peritos profesionales
              </div>
            </div>
          </div>
        )}
        ...
        {result && (pagado || !pendientePago) && (() => {
          ...
          const recMap = {
            recomendable: { t: "Compra recomendable", ic: "✅", col: C.ok, bg: "#E3F7EE" },
            con_precaucion: { t: "Comprar con precaución", ic: "⚠️", col: C.yellow, bg: "#FDEEE2" },
            no_recomendable: { t: "No recomendable", ic: "🛑", col: C.warn, bg: "#FDE9E9" },
          };
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#EAF1F8", border: `1px solid #B8D4E8`, borderRadius: 10, padding: "9px 14px", marginBottom: 16, fontSize: 12.5, color: "#16294A", lineHeight: 1.45 }}>
              <span style={{ fontFamily: mono, fontWeight: 700, color: "#F26419", textShadow: "0 0 8px rgba(45,226,230,0.6)" }}>IA</span>
            ...
            {parts.length > 0 && (<>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
                <div style={{ background: "#FDE9E9", border: `1px solid ${C.warn}`, borderRadius: 12, padding: 16, textAlign: "center" }}><div style={{ fontFamily: serif, fontSize: 32, fontWeight: 700, color: C.warn, lineHeight: 1 }}>{nA}</div><div style={{ fontSize: 12, color: C.sub, marginTop: 6 }}>🔴 Urgentes</div></div>
                <div style={{ background: "#FDEEE2", border: `1px solid ${C.yellow}`, borderRadius: 12, padding: 16, textAlign: "center" }}><div style={{ fontFamily: serif, fontSize: 32, fontWeight: 700, color: C.yellow, lineHeight: 1 }}>{nM}</div><div style={{ fontSize: 12, color: C.sub, marginTop: 6 }}>🟡 Medio plazo</div></div>
                <div style={{ background: "#E3F7EE", border: `1px solid ${C.ok}`, borderRadius: 12, padding: 16, textAlign: "center" }}><div style={{ fontFamily: serif, fontSize: 32, fontWeight: 700, color: C.ok, lineHeight: 1 }}>{nB}</div><div style={{ fontSize: 12, color: C.sub, marginTop: 6 }}>🟢 Preventivos</div></div>
              </div>
            </>)}
            ...
            {(cmp.precio_pedido || cmp.precio_mercado) && (<>
              ...
              <div style={card}><div style={{ padding: "20px 22px" }}>
                ...
                {typeof cmp.diferencia_pct === "number" && (
                  <span style={{ display: "inline-block", background: cmp.diferencia_pct <= 0 ? "#E3F7EE" : "#FDE9E9", color: cmp.diferencia_pct <= 0 ? C.ok : C.warn, border: `1px solid ${cmp.diferencia_pct <= 0 ? C.ok : C.warn}`, borderRadius: 999, padding: "4px 12px", fontWeight: 700, fontSize: 13, marginTop: 6 }}>
              ...
              <div style={card}>
                {prevs.map((p, i, arr) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", borderBottom: i < arr.length - 1 ? `1px solid ${C.line}` : "none" }}>
                    {p.plazo && <div style={{ fontFamily: mono, fontSize: 11, color: "#F26419", background: "#EAF1F8", border: "1px solid #B8D4E8", borderRadius: 8, padding: "5px 9px", whiteSpace: "nowrap", minWidth: 68, textAlign: "center" }}>{p.plazo}</div>}
            ...
            {(escen.length > 0 || neg.mensaje_vendedor) && (<>
                  ...
                  {escen.map((e, i) => {
                    ...
                    return (
                      <div key={i} style={{ background: C.card, border: `1px solid ${mid ? C.orange : C.line}`, boxShadow: mid ? `0 0 0 1px ${C.orange}` : "none", borderRadius: 12, padding: 16, textAlign: "center" }}>
                        ...
                        {mid && <div style={{ display: "inline-block", marginTop: 8, fontSize: 10, fontFamily: mono, background: "#FDEEE2", color: C.orange, border: `1px solid ${C.orange}`, borderRadius: 6, padding: "2px 7px" }}>RECOMENDADA</div>}
              ...
              {neg.mensaje_vendedor && (
                <div style={{ background: C.bg2, border: `1px dashed ${C.line}`, borderRadius: 12, padding: "18px 20px", marginTop: 14 }}>
                  <div style={{ fontFamily: mono, fontSize: 10.5, color: "#F26419", letterSpacing: 0.5, marginBottom: 8, textTransform: "uppercase" }}>✉ Mensaje listo para enviar al vendedor</div>
            ...
            {(() => {
              const fA = Array.isArray(result.__fotosA) && result.__fotosA.length ? result.__fotosA : prevA;
              const fV = Array.isArray(result.__fotosV) && result.__fotosV.length ? result.__fotosV : prevV;
              if (!fA.length && !fV.length) return null;
              return (<>
                <h2 style={sec}>Fotos de la vivienda</h2>
                {fA.length > 0 && (<>
                  <div style={{ fontFamily: mono, fontSize: 11, fontWeight: 700, color: C.sub, letterSpacing: 0.5, marginBottom: 8, textTransform: "uppercase" }}>Del anuncio</div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 8, marginBottom: 18 }}>
                    {fA.map((u, i) => (<img key={"a" + i} src={u} alt={"Foto del anuncio " + (i + 1)} style={{ width: "100%", height: 120, objectFit: "cover", borderRadius: 10, border: `1px solid ${C.line}` }} />))}
                  </div>
                </>)}
                {fV.length > 0 && (<>
                  <div style={{ fontFamily: mono, fontSize: 11, fontWeight: 700, color: C.orange, letterSpacing: 0.5, marginBottom: 8, textTransform: "uppercase" }}>De tu visita</div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 8 }}>
                    {fV.map((u, i) => (<img key={"v" + i} src={u} alt={"Foto de la visita " + (i + 1)} style={{ width: "100%", height: 120, objectFit: "cover", borderRadius: 10, border: `1px solid ${C.line}` }} />))}
                  </div>
                </>)}
              </>);
            })()}
      ...
      <div className="guia-print" style={{ maxWidth: 720, margin: "0 auto", padding: "32px", fontFamily: sans, color: "#111" }}>
        <div style={{ borderBottom: "2px solid #E23B3B", paddingBottom: 12, marginBottom: 20 }}>
        ...
        {GUIA.pruebas.map((p) => (
          <div key={p} style={{ display: "flex", gap: 8, fontSize: 13, padding: "3px 0", breakInside: "avoid" }}>
            <span style={{ display: "inline-block", width: 12, height: 12, border: "1.5px solid #E23B3B", borderRadius: 3, marginTop: 2, flexShrink: 0 }} />
            <span>{p}</span>
          </div>
        ))}
