...
A) PARTIDAS VALORADAS (presupuesto tipo obra) de reformas/gastos detectados SOLO en las imágenes o notas. Cada partida: concepto, unidad, cantidad, precio_unitario (€), total (€), urgencia.
          ...
          if (dentro) {
            ...
            if (c === '"') {
              ...
              const cierre =
                ...
                /^\s*,\s*"(?:[^"\\]|\\.)*"\s*:/.test(resto) ||
                /^\s*,\s*[}\]]/.test(resto);
              if (cierre) { out += '"'; dentro = false; } else { out += '\\"'; }
              continue;
            }
            out += c; continue;
          }
          if (c === '"') { dentro = true; out += c; continue; }
          if (c === "{" || c === "[") { pila.push(c); out += c; continue; }
          if (c === "}" || c === "]") { pila.pop(); out += c; continue; }
          out += c;
        }
        if (dentro) out += '"';
        out = out.replace(/[,:]\s*$/, "");
        while (pila.length) { const a = pila.pop(); out += a === "{" ? "}" : "]"; }
