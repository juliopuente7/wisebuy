...
export default function CosteRealVivienda() {
  ...
  //    Cuando quieras cobrar de verdad, ponlo en false.
...
OBLIGATORIO: rellena SIEMPRE estos campos, nunca los dejes vacíos: cada partida DEBE tener "urgencia" ("alta", "media" o "baja"), "total" (número, = cantidad × precio_unitario), "detalle", "consecuencia" (qué pasa si no se arregla) y "gremio" (a quién llamar). "sobrecoste_total_estimado" DEBE ser la suma de los totales de las partidas. "coste_real_estimado" = precio_venta + sobrecoste_total_estimado. "veredicto" DEBE ser "buena", "ajustada" o "cara", con "veredicto_texto" explicándolo. "recomendacion" DEBE ser "recomendable", "con_precaucion" o "no_recomendable", con "recomendacion_razones" (3 razones). Rellena "costes_compra" (impuestos ITP/IVA según sea 2ª mano u obra nueva y comunidad autónoma, notaría/registro/gestoría, total) y "gastos_anuales" (IBI, comunidad, suministros estimados) con importes orientativos realistas. Rellena "comparativa_precio" con precio_pedido, precio_mercado (m² × precio/m² de la zona), diferencia_pct y un texto. Si no hay reformas, pon partidas:[] y sobrecoste_total_estimado:0.`;
      ...
      const parsed = repararJSON(texto);
      if (!parsed) {
        const t = (texto || "").trim();
      ...
      setResult(parsed);
      setTab("informe");
      if (!pagado && !MODO_PRUEBAS) setPendientePago(true);
      window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (e) {
      console.error(e);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.ink, fontFamily: sans }}>
      ...
      <div className="app-main" style={{ maxWidth: 960, margin: "0 auto", padding: "44px 28px 96px" }}>
        {!result && (
          <>
            ...
            <header style={{ marginBottom: 30 }}>
              ...
              <h1 style={{ fontFamily: serif, fontSize: "clamp(30px, 5.5vw, 46px)", fontWeight: 600, lineHeight: 1.08, letterSpacing: -0.8, margin: 0 }}>
                Visita tu futura casa con <span style={{ color: C.yellow }}>criterio</span>
              </h1>
          ...
          <div>
            <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, overflow: "hidden" }}>
              ...
              <div style={{ padding: "22px 24px" }}>
                ...
                {result.recomendacion && (() => {
                  const R = {
                    recomendable: { l: "COMPRA RECOMENDABLE", ico: "✅", col: C.ok, bg: "#18271F" },
                    con_precaucion: { l: "COMPRA CON PRECAUCIÓN", ico: "⚠️", col: C.yellow, bg: "#332B15" },
                    no_recomendable: { l: "NO RECOMENDABLE A ESTE PRECIO", ico: "🛑", col: C.warn, bg: "#3A2420" },
                  }[result.recomendacion] || null;
                  if (!R) return null;
                  return (
                    <div style={{ marginBottom: 14, background: R.bg, border: `1px solid ${R.col}66`, borderRadius: 12, padding: "13px 15px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                        <span style={{ fontSize: 20 }}>{R.ico}</span>
                        <span style={{ fontFamily: mono, fontWeight: 700, fontSize: 14, letterSpacing: 0.8, color: R.col }}>{R.l}</span>
                      </div>
                      {Array.isArray(result.recomendacion_razones) && result.recomendacion_razones.length > 0 && (
                        <ul style={{ margin: "9px 0 0", paddingLeft: 18 }}>
                          {result.recomendacion_razones.map((r, i) => (
                            <li key={i} style={{ fontSize: 12.5, color: C.ink, lineHeight: 1.5, marginBottom: 3, opacity: 0.92 }}>{r}</li>
                          ))}
                        </ul>
                      )}
                    </div>
                ...
                {result.veredicto && (() => {
                  const V = {
                    buena: { l: "BUENA OFERTA", ico: "✅", col: C.ok, bg: "#18271F" },
                    ajustada: { l: "PRECIO AJUSTADO", ico: "⚖️", col: C.yellow, bg: "#332B15" },
                    cara: { l: "CARA PARA LA ZONA", ico: "⚠️", col: C.warn, bg: "#3A2420" },
                  }[result.veredicto] || null;
                  if (!V) return null;
                  return (
                    <div style={{ marginTop: 16, background: V.bg, border: `1px solid ${V.col}66`, borderRadius: 12, padding: "14px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                        <span style={{ fontSize: 18 }}>{V.ico}</span>
                        <span style={{ fontFamily: mono, fontWeight: 700, fontSize: 13, letterSpacing: 1, color: V.col }}>{V.l}</span>
                      </div>
                      {(result.precio_m2_zona || result.precio_mercado_estimado) && (
                        <div style={{ display: "flex", gap: 16, marginBottom: 8, flexWrap: "wrap" }}>
                          {result.precio_m2_zona ? <span style={{ fontSize: 12, color: C.sub, fontFamily: mono }}>Zona: <b style={{ color: C.ink }}>{fmt(result.precio_m2_zona)} €/m²</b></span> : null}
                          {result.precio_mercado_estimado ? <span style={{ fontSize: 12, color: C.sub, fontFamily: mono }}>Valor de mercado: <b style={{ color: C.ink }}>~{fmt(result.precio_mercado_estimado)} €</b></span> : null}
                        </div>
                      )}
                      {result.veredicto_texto && <div style={{ fontSize: 13, color: C.ink, lineHeight: 1.5, opacity: 0.92 }}>{result.veredicto_texto}</div>}
                    </div>
                  );
                })()}
                {result.comparativa_precio && (result.comparativa_precio.precio_mercado || result.comparativa_precio.texto) && (() => {
                  const cp = result.comparativa_precio;
                  const dif = Number(cp.diferencia_pct);
                  const bajo = !isNaN(dif) && dif < 0;
                  const col = bajo ? C.ok : (dif > 0 ? C.warn : C.yellow);
                  return (
                    <div style={{ marginTop: 14, background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: "14px 16px" }}>
                      <div style={{ fontFamily: mono, fontSize: 11, color: C.faint, letterSpacing: 1, marginBottom: 10 }}>PRECIO vs MERCADO DE LA ZONA</div>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
                        <div><div style={{ fontSize: 10.5, color: C.sub, fontFamily: mono }}>PIDEN</div><div style={{ fontFamily: serif, fontSize: 19, fontWeight: 600 }}>{fmt(cp.precio_pedido)} €</div></div>
                        <div style={{ textAlign: "right" }}><div style={{ fontSize: 10.5, color: C.sub, fontFamily: mono }}>MERCADO ZONA</div><div style={{ fontFamily: serif, fontSize: 19, fontWeight: 600 }}>~{fmt(cp.precio_mercado)} €</div></div>
                      </div>
                      {!isNaN(dif) && (
                        <div style={{ display: "inline-block", fontSize: 12.5, fontWeight: 700, color: col, background: `${col}22`, border: `1px solid ${col}55`, borderRadius: 20, padding: "3px 12px" }}>
                          {bajo ? "▼" : (dif > 0 ? "▲" : "=")} {Math.abs(dif)}% {bajo ? "por debajo" : (dif > 0 ? "por encima" : "en línea")} del mercado
                        </div>
                      )}
                      {cp.texto && <div style={{ fontSize: 12.5, color: C.ink, lineHeight: 1.5, marginTop: 9, opacity: 0.9 }}>{cp.texto}</div>}
                    </div>
                  );
                })()}
                {(result.costes_compra || result.gastos_anuales) && (
                  <div style={{ marginTop: 14, background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: "14px 16px" }}>
                    <div style={{ fontFamily: mono, fontSize: 11, color: C.faint, letterSpacing: 1, marginBottom: 10 }}>LO QUE CUESTA DE VERDAD (aparte del precio)</div>
                    {result.costes_compra && (
                      <div style={{ marginBottom: result.gastos_anuales ? 12 : 0 }}>
                        <div style={{ fontSize: 12, fontWeight: 600, color: C.orange, marginBottom: 6 }}>Gastos de compra (una vez)</div>
                        {result.costes_compra.itp_iva ? <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, padding: "2px 0", color: C.ink }}><span>Impuestos (ITP/IVA)</span><span style={{ fontFamily: mono }}>{fmt(result.costes_compra.itp_iva)} €</span></div> : null}
                        {result.costes_compra.notaria_registro_gestoria ? <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, padding: "2px 0", color: C.ink }}><span>Notaría, registro y gestoría</span><span style={{ fontFamily: mono }}>{fmt(result.costes_compra.notaria_registro_gestoria)} €</span></div> : null}
                        {result.costes_compra.total_gastos_compra ? <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "5px 0 0", marginTop: 4, borderTop: `1px solid ${C.line}`, fontWeight: 700, color: C.yellow }}><span>Total gastos de compra</span><span style={{ fontFamily: mono }}>{fmt(result.costes_compra.total_gastos_compra)} €</span></div> : null}
                      </div>
                    )}
                    {result.gastos_anuales && result.gastos_anuales.total_anual_estimado ? (
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 600, color: C.orange, marginBottom: 6 }}>Gastos cada año</div>
                        {result.gastos_anuales.ibi ? <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, padding: "2px 0", color: C.ink }}><span>IBI</span><span style={{ fontFamily: mono }}>{fmt(result.gastos_anuales.ibi)} €</span></div> : null}
                        {result.gastos_anuales.comunidad_estimada ? <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, padding: "2px 0", color: C.ink }}><span>Comunidad (estim.)</span><span style={{ fontFamily: mono }}>{fmt(result.gastos_anuales.comunidad_estimada)} €</span></div> : null}
                        {result.gastos_anuales.suministros_estimados ? <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, padding: "2px 0", color: C.ink }}><span>Suministros (estim.)</span><span style={{ fontFamily: mono }}>{fmt(result.gastos_anuales.suministros_estimados)} €</span></div> : null}
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "5px 0 0", marginTop: 4, borderTop: `1px solid ${C.line}`, fontWeight: 700, color: C.yellow }}><span>Total al año</span><span style={{ fontFamily: mono }}>{fmt(result.gastos_anuales.total_anual_estimado)} €</span></div>
                      </div>
                    ) : null}
                    {result.costes_compra?.nota && <div style={{ fontSize: 11.5, color: C.sub, marginTop: 10, lineHeight: 1.45 }}>{result.costes_compra.nota}</div>}
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ marginTop: 22 }}>
              ...
              <div className="tab-body" style={{ display: tab === "informe" ? "block" : "none" }}>
                {result.partidas?.length > 0 && (() => {
                  const na = result.partidas.filter(p => p.urgencia === "alta").length;
                  const nm = result.partidas.filter(p => p.urgencia === "media").length;
                  const nb = result.partidas.filter(p => (p.urgencia || "baja") === "baja").length;
                  const chip = (n, txt, col, bg) => (
                    <div style={{ flex: 1, minWidth: 90, background: bg, border: `1px solid ${col}55`, borderRadius: 11, padding: "11px 8px", textAlign: "center" }}>
                      <div style={{ fontFamily: serif, fontSize: 24, fontWeight: 700, color: col, lineHeight: 1 }}>{n}</div>
                      <div style={{ fontSize: 10.5, color: C.sub, marginTop: 4, letterSpacing: 0.3 }}>{txt}</div>
                    </div>
                  );
                  return (
                    <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                      {chip(na, "🔴 Urgente", C.warn, "#3A2420")}
                      {chip(nm, "🟡 Medio plazo", C.yellow, "#332B15")}
                      {chip(nb, "🟢 Preventivo", C.ok, "#18271F")}
                    </div>
                  );
                })()}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {result.partidas?.map((p, i) => {
                    ...
                    return (
                      <div key={i} style={{ background: u.bg, border: `1px solid ${u.bd}55`, borderLeft: `4px solid ${u.bd}`, borderRadius: 12, padding: "13px 15px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 12 }}>
                          <div style={{ fontWeight: 600, fontSize: 14.5, lineHeight: 1.3 }}>{p.concepto}</div>
                          <div style={{ fontFamily: mono, fontWeight: 700, fontSize: 16, color: C.orange, whiteSpace: "nowrap" }}>{fmt(p.total)} €</div>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 6, flexWrap: "wrap" }}>
                          <span style={{ fontSize: 11.5, fontFamily: mono, color: C.sub }}>{fmt(p.cantidad)} {p.unidad} × {fmt(p.precio_unitario)} €</span>
                          <span style={{ fontSize: 11, fontWeight: 700, color: u.c, background: `${u.bd}22`, border: `1px solid ${u.bd}55`, borderRadius: 20, padding: "2px 9px", letterSpacing: 0.3 }}>{u.icon} {u.l}</span>
                        </div>
                        {p.detalle && (
                          <div style={{ marginTop: 8, fontSize: 12.5, color: C.ink, lineHeight: 1.5, opacity: 0.9, borderTop: `1px solid ${u.bd}22`, paddingTop: 8 }}>{p.detalle}</div>
                        )}
                        {(p.consecuencia || p.gremio) && (
                          <div style={{ marginTop: 7, display: "flex", flexDirection: "column", gap: 3 }}>
                            {p.consecuencia && <div style={{ fontSize: 11.8, color: C.sub, lineHeight: 1.4 }}><b style={{ color: u.c }}>Si no se arregla:</b> {p.consecuencia}</div>}
                            {p.gremio && <div style={{ fontSize: 11.8, color: C.sub }}><b style={{ color: C.ink }}>A quién llamar:</b> {p.gremio}</div>}
                          </div>
                        )}
                      </div>
                    );
                  })}
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 18px", background: `linear-gradient(135deg, ${C.orangeDeep}, ${C.yellowDeep})`, borderRadius: 12, marginTop: 2 }}>
                    <span style={{ fontWeight: 700, fontSize: 13.5, fontFamily: mono, letterSpacing: 0.5, color: "#1A1815" }}>TOTAL REFORMAS</span>
                    <span style={{ fontFamily: serif, fontWeight: 700, fontSize: 22, color: "#1A1815" }}>{fmt(result.sobrecoste_total_estimado)} €</span>
                  </div>
                </div>
