...
Todo en español claro para un comprador normal.`;
      ...
      // Extraer el JSON del texto recibido (por si viene con algún envoltorio o cortado).
      let limpio = texto.replace(/```json|```/g, "").trim();
      let parsed = null;
      const intentar = (txt) => { try { return JSON.parse(txt); } catch (_) { return null; } };
      // Sustituye saltos de línea / tabuladores crudos por espacios (rompen el JSON si están dentro de un texto)
      const sanear = (txt) => txt.replace(/[\r\n\t]+/g, " ");
      // 1) tal cual
      parsed = intentar(limpio);
      // 1b) saneado
      if (!parsed) parsed = intentar(sanear(limpio));
      // 2) recortando desde el primer { hasta el último }
      if (!parsed) {
        const ini = limpio.indexOf("{"); const fin = limpio.lastIndexOf("}");
        if (ini !== -1 && fin !== -1 && fin > ini) {
          const trozo = limpio.slice(ini, fin + 1);
          parsed = intentar(trozo) || intentar(sanear(trozo));
        }
      }
      // 3) si vino cortado (faltan cierres), intentar repararlo
      if (!parsed) {
        const ini = limpio.indexOf("{");
        if (ini !== -1) {
          let frag = sanear(limpio.slice(ini));
          // si hay un número impar de comillas, cerrar la cadena colgando
          const comillas = (frag.match(/(?<!\\)"/g) || []).length;
          if (comillas % 2 !== 0) frag += '"';
          // quitar coma o dos puntos colgando al final
          frag = frag.replace(/[,:]\s*$/, "");
          const abiertas = (frag.match(/{/g) || []).length - (frag.match(/}/g) || []).length;
          const corch = (frag.match(/\[/g) || []).length - (frag.match(/\]/g) || []).length;
          if (corch > 0) frag += "]".repeat(corch);
          if (abiertas > 0) frag += "}".repeat(abiertas);
          parsed = intentar(frag);
        }
      }
      if (!parsed) {
        const muestra = (texto || "").trim().slice(0, 200);
        throw new Error("La IA respondió pero no en el formato esperado. Empieza así: «" + muestra + "»");
      }
